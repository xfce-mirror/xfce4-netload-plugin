...
void get_current_netload(netdata* data, unsigned long *in, unsigned long *out, unsigned long *tot) 
{
    ...
    {
        data->cur_in = (gint64)( data->stats.rx_bytes / delta_t + 0.5);
    }
    else
    {
        data->cur_in = (gint64)( (data->stats.rx_bytes - data->backup_in) / delta_t + 0.5);
    }
	
    if (data->backup_out > data->stats.tx_bytes)
    {
        data->cur_out = (gint64)( data->stats.tx_bytes / delta_t + 0.5);
    }
    else
    {
        data->cur_out = (gint64)( (data->stats.tx_bytes - data->backup_out) / delta_t + 0.5);
    }
