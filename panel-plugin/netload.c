        ...
        while (fgets(buffer, sizeof(buffer), fp) != NULL) {
            ...
            if (colon != NULL) {
                *colon = '\0';
                ...
                gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(global->monitor->net_combo), start);
                if (!strcmp(start, global->monitor->options.network_device)) {
                    gtk_combo_box_set_active(GTK_COMBO_BOX(global->monitor->net_combo), count);
                }
                ++count;
            }
        }
